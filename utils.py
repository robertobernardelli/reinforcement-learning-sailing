...
def collision_with_boundaries(boat_position):
    if boat_position[0]>=500 or boat_position[0]<0 or boat_position[1]>=500 or boat_position[1]<0 :
        return 1
    else:
        return 0
...
def get_line_params(wind_direction):
    ...
    '''
    return (70, 40), (70 + int(60 * np.sin(np.deg2rad(wind_direction))), 
...
def draw_boat_rectangle(img, boat_position):
    ...
    '''
    cv2.rectangle(img,(int(boat_position[0]),int(boat_position[1])),
                  (int(boat_position[0])+1,int(boat_position[1])+1),(0,255,0),3)
    return img
...
def draw_sea_and_dashboard(wind_direction, boat_heading, boat_speed, total_reward):
    ...
    img = cv2.arrowedLine(img, start_point, end_point,
                                    (255,255,255), 1, tipLength=0.2)
    
    label_wind= f'wind: {int(wind_direction)} deg'
    img = cv2.putText(img, label_wind, (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 
                      0.5, (255,255,255), 1, cv2.LINE_AA)
    
    label_boat= f'boat: {round(float(boat_speed), 1)} knts, {(180-boat_heading)%360} deg'
    img = cv2.putText(img, label_boat, (150, 15), cv2.FONT_HERSHEY_SIMPLEX,
                      0.5, (255,255,255), 1, cv2.LINE_AA)
    
    label_reward= f'reward: {round(float(total_reward),2)}'
    img = cv2.putText(img, label_reward, (370, 15), cv2.FONT_HERSHEY_SIMPLEX,
                      0.5, (255,255,255), 1, cv2.LINE_AA)
